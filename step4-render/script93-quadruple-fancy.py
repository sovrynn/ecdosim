...
TEXT_LABEL_TL = "ECDOsim v10 (Bonus Scenario: S1 -> S2 @2x Strength w/ Extended Drainage)"
...
def add_text_overlays(img, tl, tr, bl, br, line_spacing: int = 0):
    ...
    if tr and tr.get("texts"):
        ...
        for line in tr["texts"]:
            text_w, text_h = _text_size(tr["font"], line)
    ...
    if bl and bl.get("texts"):
        heights = [_text_size(bl["font"], line)[1] for line in bl["texts"]]
        if heights:
            total_block_height = sum(heights) + line_spacing * (len(heights) - 1)
            x = bl["offset_x"]
            y = H - bl["offset_y"] - total_block_height
            for line, h in zip(bl["texts"], heights):
                draw.text((x, y), line, font=bl["font"], fill=bl["color"])
                y += h + line_spacing

    # --- Bottom-right ---
    if br and br.get("texts"):
        heights = [_text_size(br["font"], line)[1] for line in br["texts"]]
        if heights:
            total_block_height = sum(heights) + line_spacing * (len(heights) - 1)
            y = H - br["offset_y"] - total_block_height
            for line, h in zip(br["texts"], heights):
