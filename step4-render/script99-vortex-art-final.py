...
REVERSE_STAGE1_MULT = 0.5  # initial seed-frame hold (brightened)
REVERSE_STAGE2_MULT = 3  # reverse zig-zag "windback" (brightened)
REVERSE_STAGE3_MULT = 0.5  # initial first-frame hold (brightened)

# New: which input frame to use as the reverse "seed" (1-based index).
# If None, default to the last frame in the sequence.
REVERSE_SEED_FRAME = 58  # e.g. 35 to start from frame 35
...
def generate_reverse_stage2_indices(max_index: int, stage2_frames: int, start_index: int = None):
    """
    Generate indices for Reverse Stage 2, following the pattern:
    ...
    i.e. backwards sequence: 1, 3, 2, 5, 4, 7, 6, ...

    We:
      * Build that sequence in "backwards order".
      * Map each number into [1..max_index] using modulo.
      * Skip candidates that would repeat the previous index (so no identical
        frames are adjacent).
      * Stop once we have `stage2_frames` indices.
      * Return the forward order (so the last element is 1, second last 3, etc.).
    """
    if stage2_frames <= 0:
        return []

    domain_max = max(1, max_index)

    backwards = []
    prev = None
    j = 0  # pattern index: 0 -> 1, then (3,2), (5,4), ...

    while len(backwards) < stage2_frames:
        if j == 0:
            raw = 1
        else:
            k = (j - 1) // 2 + 1
        ...
        # Map into [1..domain_max] cyclically so we can keep going
        mapped = ((raw - 1) % domain_max) + 1

        # Enforce: no two adjacent indices the same in the *backwards* list
        if mapped == prev:
            continue

        backwards.append(mapped)
        prev = mapped

    # Now backwards[0] is the last frame of Stage 2, etc.
    # Forward order is reversed.
    indices = list(reversed(backwards))
...
def run_reverse_sequence(input_paths, out_dir, start_frame_index):
    ...
    # Stage 2: reverse path with your custom backwards pattern, brightened
    frames_stage2 = int(FPS * REVERSE_STAGE2_MULT)
    if frames_stage2 > 0:
        debug(
            f"\n=== Reverse Stage 2 (custom backwards pattern ending at frame 1) "
