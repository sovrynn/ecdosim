...
Next, the DEM is scaled from rgb to a 0->1 grayscale band, and the tif file is modified to have 4 rgb-opacity bands to have 1 grayscale band. (script 6)

In later sim versions the DEM is chopped off at the bottom (concatenate -t 0.18) which is used for domain optimization (and only removes a tiny fraction of ocean volume over << 1% of the surface area) (script 7)
