...
# Number of pixels to inset the radius of the circle
INSET_PIXELS = 5  # <-- set this to whatever inset you want


def compute_circle_mask(height, width, inset_pixels=0):
    """
    Create a boolean mask for a circle centered in the image.
    The base circle touches the closest border and is fully contained,
    then the radius is reduced by `inset_pixels`.
    """
    ...
    base_radius = min(dist_left, dist_right, dist_top, dist_bottom)

    # Final radius after insetting
    radius = max(0.0, base_radius - inset_pixels)
...
def process_geotiff(input_path: Path):
    ...
    with rasterio.open(input_path) as src:
        ...
        # Compute circle mask with inset (True = keep, False = set to white)
        mask_inside = compute_circle_mask(height, width, inset_pixels=INSET_PIXELS)
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Circle-crop a GeoTIFF with optional inset.\n"
            "- For single-band grayscale: keeps an inset center circle and sets outside to white.\n"
            "- For 4-band RGB(A): keeps an inset center circle, sets RGB outside to white."
        )
    )
    parser.add_argument(
        "input_geotiff",
