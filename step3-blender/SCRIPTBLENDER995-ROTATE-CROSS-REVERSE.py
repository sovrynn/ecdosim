...
# Degrees per accumulated unit of vortex strength (now used SUBTRACTIVELY)
SCALE_DEGREES = 1.0

# Hardcoded subtractive value (degrees) for the 10th keyframe
...
def main():
    ...
    # Base rotations (degrees) at FIRST FRAME
    ...
    # Determine placement frames: up to 9 zero-crossings from k2→k3 onward,
    # skipping the segment that starts at the second-to-last vortex keyframe.
    placement_frames = []
    ...
    # Insert keyframes (SUBTRACTIVE logic)
    for idx, f in enumerate(placement_frames):
        if idx == 9:
            # 10th keyframe: subtract fixed 104°
            delta_deg = -HARDCODED_TENTH_DEGREES
            src_info = "hardcoded 10th"
        else:
            cum = cumulative_at(f, C, C_start, fcurve)
            delta_deg = -(SCALE_DEGREES * cum)  # SUBTRACTIVE
            src_info = f"-(SCALE * cumulative) = {delta_deg:.3f}°"

        z1_deg = base_z_cross1_deg + delta_deg
        z2_deg = base_z_cross2_deg + delta_deg
        ...
        print(f"[Keyframe {idx+1}] frame={f:.4f} | delta={delta_deg:.3f}° ({src_info}) | "
              f"cross1.z={z1_deg:.3f}° | cross2.z={z2_deg:.3f}°")

    print(f"Placed {len(placement_frames)} rotation keyframes (SUBTRACTIVE; SCALE={SCALE_DEGREES}°/unit; 10th=-{HARDCODED_TENTH_DEGREES}°).")
