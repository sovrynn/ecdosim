...
def eval_strength_at_frame(obj, frame):
    ...
    # Reading the property after frame_set gives the interpolated (bezier) value.
...
# ----------------------------
set_key_z_deg(cross1, 1, 0.0)    # cross1 at 0°
set_key_z_deg(cross2, 1, 90.0)   # cross2 at 90°

# Read (or set) baselines after inserting the initial keys
...
if START_FRAME < 2:
    START_FRAME = 2  # ensure we have a previous frame to compare against
...
for f in range(START_FRAME, frame_end + 1):
    ...
    crossed = False
    # Exactly zero at an integer frame OR sign change from previous frame
    if val == 0.0:
        crossed = True
    ...
    if crossed:
        # Additive is the running sum (already scaled); treat as DEGREES
        add_deg = running_sum

        # cross1
        new_z_deg_1 = base_z_deg_cross1 + add_deg
        set_key_z_deg(cross1, f, new_z_deg_1)
        print(f"[cross1] frame={f}  z0(deg)={base_z_deg_cross1:.6f}  +add(deg)={add_deg:.6f}  => z_new(deg)={new_z_deg_1:.6f}")

        # cross2
        new_z_deg_2 = base_z_deg_cross2 + add_deg
        set_key_z_deg(cross2, f, new_z_deg_2)
        print(f"[cross2] frame={f}  z0(deg)={base_z_deg_cross2:.6f}  +add(deg)={add_deg:.6f}  => z_new(deg)={new_z_deg_2:.6f}")
...
# Optional: return timeline to its original current frame (nice-to-have)
